...
pub fn evaluate_head_promotion(
    ...
    if eligibility != HeadEligibility::Full {
        return HeadPromotionDecision::Deny(
            "device was not explicitly authorized as a full Head candidate",
        );
    }
    ...
    if !matches!(
        memory_mode,
        MemoryDeploymentMode::CanonicalService | MemoryDeploymentMode::FullReplicaCandidate
    ) {
        return HeadPromotionDecision::Deny(
            "device does not have a complete canonical memory profile",
        );
    }
    if !canonical_data_ready {
        return HeadPromotionDecision::Deny("canonical data is not ready");
    }
    if !fencing_ready {
        return HeadPromotionDecision::Deny("authority fencing is not ready");
    }
...
mod tests {
    ...
    fn default_client_cannot_become_head() {
        assert!(matches!(
            evaluate_head_promotion(
                HeadEligibility::None,
                AuthenticationAssurance::Strong,
                MemoryDeploymentMode::ClientCache,
                true,
                true
            ),
    ...
    fn explicit_full_candidate_can_become_head_when_ready() {
        assert_eq!(
            evaluate_head_promotion(
                HeadEligibility::Full,
                AuthenticationAssurance::Strong,
                MemoryDeploymentMode::FullReplicaCandidate,
                true,
                true
            ),
            HeadPromotionDecision::Allow
