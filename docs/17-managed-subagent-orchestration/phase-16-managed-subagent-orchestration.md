...
Status: owner note for the current managed-subagent contract-completion and Stage 8 operator-surface slice.
...
- the CLI exposes a bounded operator surface through `subagent-launch`, `subagent-list`, `subagent-show`, `subagent-wait`, `subagent-record-control`, and `subagent-close`;
...
The current worker/reviewer/final-review slice launches a managed child by:
...
6. accepting bounded parent control messages through `send`, including cancel and budget-tightening state transitions.
...
- worker/reviewer/final-review task-package snapshots;
- durable parent/child lineage for managed launches;
- bounded operator commands for launch-and-wait, list, show, wait/reconcile, record-control, and close;
...
- сервис поддерживает роли `worker`, `reviewer` и `final_review`;
- сервис поддерживает `launch`, `wait`, `send` и `close`;
...
- CLI exposes bounded operator surface через `subagent-launch`, `subagent-list`, `subagent-show`, `subagent-wait`, `subagent-record-control` и `subagent-close`;
...
В текущем worker/reviewer/final-review slice managed child запускается так:
...
5. parent может вызвать `wait` для terminal state или non-terminal update, затем явно `close` boundary;
6. bounded parent control messages принимаются через `send`, включая cancel и budget-tightening state transitions.
...
- bounded operator commands для launch-and-wait, list, show, wait/reconcile, record-control и close;
