# Dennett Agent, Voice, Capability and Integration Architecture
...
**Каноническое имя:** `82_Dennett_Agent_Voice_Capability_and_Integration_Architecture.md`

Этот документ является самостоятельным. Он объясняет архитектуру интеллектуального и интеграционного слоя Dennett так, чтобы читатель мог понять её после общего описания продукта, не зная историю обсуждения и прежние варианты решений.
...
> **Как Dennett превращает намерение и контекст в работу моделей и агентов, подключает быстро меняющиеся внешние возможности, сохраняет нативные преимущества провайдеров, контролирует инструменты и внешние эффекты, обеспечивает живой голосовой режим и остаётся тестируемым, заменяемым и пригодным для продакшена?**

Документ продолжает и не переопределяет:

- функциональную концепцию Dennett; [[S01]]
...
Технологические продукты в тексте являются **адаптерами или кандидатами**, а не источниками истины Dennett. Их текущие возможности датированы и обязаны подтверждаться capability probes. Архитектурные инварианты не должны зависеть от того, какая модель или SDK популярны в конкретный год.
...
Лучшее решение для Dennett — **Stable Intelligence Core + Native Runtime Adapters**.

Стабильное ядро Dennett определяет:
...
flowchart LR
    ...
    RT --> GENERIC[Dennett Generic Agent Loop]
...
Dennett использует это через нативный adapter.
...
Если provider session потеряна, Dennett может создать новую и восстановить работу из собственного состояния. При этом система честно отмечает потерю нативной continuity, а не делает вид, что скрытое состояние полностью мигрировало между моделями.
...
Subagent создаётся только после marginal-utility gate из Agentic Control Fabric. Внутренние subagents используют лёгкий Dennett protocol или provider-native delegation. A2A предназначен прежде всего для внешних самостоятельных агентов, а не для каждого внутреннего помощника.
...
Голосовой backend может быть OpenAI Realtime, Gemini Live, LiveKit/Pipecat pipeline или полностью локальная цепочка. Voice business state остаётся Dennett-native.

### 0.6. Structured-first computer use

При управлении приложениями Dennett предпочитает:
...
Common contract охватывает только то, что Dennett обязан понимать независимо от поставщика:
...
Provider-specific возможности хранятся в `native_extensions` и доступны target-specific UI/logic. Dennett не рисует несуществующие настройки и не сводит все платформы к устаревшему Chat Completions API.

### 0.8. Короткая формула

> **Dennett Intelligence Plane — это стабильный оркестрационный и контекстный контур, который выбирает минимально достаточный runtime, сохраняет каноническое состояние вне провайдера, активирует только нужные capabilities, проводит tools и внешние эффекты через системные границы, а voice, coding, MCP, computer-use и connectors подключает как заменяемые нативные адаптеры.**
...
- ранее принятые документы Dennett.
...
### 3.1. Модули внутри `dennett-head`
...
```text
dennett-adapter-host-rust
dennett-adapter-host-python
dennett-adapter-host-node
```
...
flowchart TB
  ...
  subgraph NODE[dennett-node / Device]
...
Adapter не выдаёт hidden chain-of-thought как контракт Dennett.
...
| Runtime | Основное применение | Что сохраняем нативно | Что остаётся Dennett-native |
|---|---|---|---|
...
| Google ADK | Gemini-native agents, live/multimodal, A2A | agents/workflows, sessions, tools, A2A | user/project authority, Dennett Run and memory |
| OpenHands | isolated software work | sandboxed coding runtime, remote workspace | Project/Task authority and final verification |
| Generic Dennett loop | local/direct APIs and fallback | none beyond model endpoint | full bounded loop state |
...
- один Dennett Project Session может быть связан с одним или несколькими Codex threads;
- workspace/worktree регистрируется в Workspace Manager;
- Codex approvals отражаются в Dennett TrustBridge;
...
- выдавать Codex более широкий filesystem scope, чем Dennett grant;
...
Provider permission не заменяет Dennett grant. Если Claude слой строже, его prompt остаётся; если слабее, Dennett не ослабляет собственную границу.
...
Dennett не использует ADK graph как универсальный внутренний Task model. Adapter получает bounded assignment и возвращает artifacts/evidence.
...
Dennett использует его как runtime, но:

- workspace lease выдаётся Dennett;
- completion проверяется Dennett;
- permissions не переходят из OpenHands config автоматически;
- remote state имеет explicit lifecycle;
- artifacts возвращаются в Dennett Object/Project layer.

### 11.7. Generic Dennett agent loop
...
3. **Dennett rehydrate:** создаётся новая provider session из committed history, Context Manifest, workspace state и stable handles.
...
Cross-provider migration всегда означает новую session. Dennett переносит только собственное наблюдаемое состояние.
...
Compaction может выполняться provider-native или Dennett-side.
...
Dennett не генерирует огромный root AGENTS на каждый turn. Effective Instruction Set может materialize provider-specific managed view только при изменении revision.
...
- Dennett-native Instruction Items;
...
Dennett не требует и не хранит hidden chain-of-thought как интеграционный интерфейс.
...
Свежие исследования предлагают graph- и feedback-based routing, но в Dennett они остаются optional. [[S43]] Сначала применяются простые правила и измеренные outcomes. Learned router принимается только после offline replay и shadow comparison.
...
- Dennett-native handle.

Dennett-native content остаётся canonical source, если skill Dennett-managed. Provider-native user-owned package сохраняет свой ownership.
...
### 31.1. Роль Dennett как MCP Host

Dennett Head/Adapter Host выступает MCP host/client. Каждый server connection изолирован и связан с Capability Source/Trust state.
...
A2A Task ↔ Dennett child Task/Run.
...
Agent Card signature/identity повышает provenance confidence, но не выдаёт permission. External agent получает только scoped inputs и не может передать Dennett grant обратно текстом.

### 32.6. Exposing Dennett agents
...
Personal Dennett не обязан публиковать оркестратора наружу.
...
OpenAI Agents SDK sandbox agents and provider coding sandboxes can be used, but Dennett stores:
...
Third-party native dynamic library не загружается в `dennett-head`.
...
Playwright MCP provides structured browser control through accessibility snapshots; it is useful as a provider-neutral adapter, but direct Playwright library/CLI can be more efficient where Dennett owns execution. [[S26]]
...
- environment managed by Dennett or provider;
...
UI-TARS, Agent S2-like designs and future backends implement the same `ComputerUsePort`. Agent S2’s generalist/specialist separation suggests value in separating planning and grounding for difficult GUI tasks, but Dennett does not require this internal topology from every backend. [[S38]]
...
Adapter не получает всю Dennett память. Он получает compact voice manifest и tools для обращения к strong model/project agent.

### 44.2. Gemini Live adapter

Optional native live/multimodal path через Google ecosystem. Dennett сохраняет committed turns и action boundary независимо от provider.
...
```text
ask_dennett_deliberation(question, handles, desired_depth, deadline)
```
...
Screenpipe may supply capture, OCR/accessibility and local retrieval, but Dennett imports through SensorSourcePort. Its DB is not canonical Memory.
...
Connector provides object/file operations but Dennett backup and Object Store policies remain separate.
...
Adapter maps provider approval to Dennett:

```text
provider requests approval
→ normalize requested operation
→ check existing Dennett grant
...
Provider approval text is not itself authority. Dennett inspects actual operation.
...
| Provider session lost | one Session/Run | native resume or Dennett rehydrate |
...
Use traces, metrics and logs with GenAI semantic conventions where mature, plus Dennett-specific attributes. [[S36A]]

### 64.3. Required spans

- `dennett.intent.resolve`;
- `dennett.context.compile`;
- `dennett.runtime.resolve`;
- `dennett.provider.session.start`;
- `dennett.model.turn`;
- `dennett.tool.invoke`;
- `dennett.mcp.call`;
- `dennett.computer.action`;
- `dennett.voice.turn`;
- `dennett.connector.effect`;
- `dennett.result.normalize`.
...
Lock-in ограничивается тем, что Task, committed turns, workspace, artifacts, effects и memory остаются в Dennett. Нативная continuity является ускорением, а не единственным состоянием.
...
Если visual backend не достигает threshold, capability остаётся experimental, а не блокирует Dennett.
...
`83_Dennett_Client_Operations_Testing_and_Implementation_Blueprint.md` должен уточнить:
...
**[S01] Dennett Functional Concept.** Видение продукта, project chats, voice, agents, memory, capabilities и автономность.  
`00_Dennett_Functional_Concept.md`

**[S02] Dennett Specification Index and Shared Contracts.** Ownership, sources of truth и общие envelopes.  
`01_Dennett_Specification_Index_and_Shared_Contracts.md`

**[S03] Dennett Memory Fabric 1.2.** Context Manifest, evidence, retrieval, project memory и influence boundaries.  
`10_Dennett_Memory_Fabric.md`

**[S04] Dennett Pragmatic Agentic Control Fabric 1.1.** Single-agent-first, execution levels, Tasks/Runs, delegation и completion.  
`20_Dennett_Agentic_Control_Fabric.md`

**[S05] Dennett Trust, Identity, Autonomy and Permissions.** Reference Monitor, grants, secrets, prompt injection и effects.  
`30_Dennett_Trust_Identity_Autonomy_and_Permissions.md`

**[S06] Dennett Voice and Ambient Interaction Fabric.** Voice Session, turn-taking, sidecar, ambient и multi-device semantics.  
`40_Dennett_Voice_and_Ambient_Interaction_Fabric.md`

**[S07] Dennett Capabilities, Providers and Integrations.** Capability lifecycle, providers, skills, MCP, plugins, connectors и local models.  
`41_Dennett_Capabilities_Providers_and_Integrations.md`

**[S08] Dennett Server Runtime, Events, Sync and Portability.** Head, Nodes, runtime, events, effects, failover и device channels.  
`50_Dennett_Server_Runtime_Events_Sync_and_Portability.md`

**[S09] Dennett Desktop Business Logic.** Project workbench, Inbox, Radar, voice/capture и command surface.  
`60_Dennett_Desktop_Application_Business_Logic.md`

**[S10] Dennett Mobile Business Logic.** Mobile remote, capture, approvals, interruption-safe UX и voice.  
`61_Dennett_Mobile_Application_Business_Logic.md`

**[S11] Dennett End-to-End Validation and Architecture Handoff.** Сквозные сценарии и architecture gates.  
`70_Dennett_End_to_End_Validation_and_Architecture_Handoff.md`

**[S12] Dennett Pre-Architecture Business Logic Completion.** Communication, ambient, recovery, packages и search contracts.  
`../specifications/contracts/README.md`

**[S13] Dennett System Architecture and Runtime Topology.** Process-selective modular monolith, Tauri/daemon, MemoryPort, adapters и testing.  
`80_Dennett_System_Architecture_and_Runtime_Topology.md`

**[S14] Dennett Data, Memory, Storage, Sync and Protocol Architecture.** PostgreSQL/SQLite/Object Store, gRPC/Protobuf, sync, effects, backup и packages.  
`81_Dennett_Data_Memory_Storage_Sync_and_Protocol_Architecture.md`
