# Dennett Client, Operations, Testing and Implementation Blueprint
...
**Каноническое имя:** `83_Dennett_Client_Operations_Testing_and_Implementation_Blueprint.md`.

Этот документ является самостоятельным. Читателю достаточно знать, что **Dennett** — персональная агентная операционная система с постоянным главным runtime, проектами по принципу Codex/Claude Code, долговременной памятью, регулируемой автономностью, голосовым режимом, локальными и облачными моделями, внешними интеграциями, desktop- и mobile-клиентами.

Предыдущие архитектурные тома уже определили:

- системную топологию, процессы и deployment profiles — `80_Dennett_System_Architecture_and_Runtime_Topology.md`; [[S13]]
- данные, память, хранилища, синхронизацию и протоколы — `81_Dennett_Data_Memory_Storage_Sync_and_Protocol_Architecture.md`; [[S14]]
- агентов, voice, capabilities, providers и integrations — `82_Dennett_Agent_Voice_Capability_and_Integration_Architecture.md`. [[S15]]
...
Лучшая реализационная модель Dennett:
...
- Vite;
- Tauri Rust shell;
- отдельный `dennett-node`, зарегистрированный как фоновый процесс ОС;
...
Tauri documentation рекомендует Channels для streaming data вместо потока отдельных events. Его updater создаёт подписанные bundles и не позволяет отключить проверку signature. Эти свойства используются, но не считаются полной системой update всего Dennett. [[S20]] [[S21]]
...
flowchart LR
    ...
    NODE[dennett-node]
    HEAD[dennett-head / local composition]
...
```text
dennett-object://<object-handle>/<rendition>
```
...
│   └── test/
├── specs/
│   ├── NativeDennettNode.ts
│   ├── NativeDennettVoice.ts
│   ├── NativeDennettCapture.ts
│   └── NativeDennettSystemSurfaces.ts
├── android/
├── ios/
├── maestro/
└── native-tests/
...
### 13.2. NativeDennettNode TurboModule
...
iOS не обещает произвольный вечный daemon. Если Head недоступен и приложение приостановлено, Dennett честно не выполняет неизвестные фоновые действия.
...
- `dennett-node` in versioned component directory;
...
- signed Dennett release manifest;
...
Dennett должен быть обслуживаем одним технически заинтересованным владельцем без команды SRE.
...
Официальная документация Collector описывает agent, gateway и agent-to-gateway deployment patterns; Dennett выбирает профиль по установке, а не требует отдельный collector в local-only mode. [[S33]]
...
dennett.command.id
dennett.effect.id
dennett.capability.id
dennett.provider.id
dennett.protocol.version
dennett.authority.epoch
```
...
Heartbeat не считается доказательством корректности: `dennett doctor` выполняет semantic probes.
...
dennettctl stop
dennettctl restart
dennettctl components
dennettctl devices
dennettctl runs
dennettctl effects unknown
dennettctl effects reconcile <id>
dennettctl sync status
dennettctl sync retry
dennettctl backup create
dennettctl backup verify
dennettctl backup restore-plan
dennettctl migrate status
dennettctl update check
dennettctl update apply
dennettctl update rollback
dennettctl logs
dennettctl support-bundle
dennettctl config validate
dennettctl protocol check
dennettctl scenario run <id>
```
...
cargo nextest run -E 'package(dennett-trust-core)'
```
...
Dennett needs repeatable scenarios spanning processes and faults. A full programming language would be overengineering, so baseline is a small declarative specification executed by `dennett-sim`.
...
Build Dennett according to the canonical specifications. Prefer the
...
Dennett is a personal agent operating environment: project agents work directly
...
Implement Dennett according to the canonical specifications and architecture.
...
1. docs/specifications/01_Dennett_Specification_Index...
...
Run:
- `cargo nextest run -p dennett-sync-core`
...
A feature imports `DennettClient`, not `@tauri-apps/api` or native module directly.
...
## Internal Dennett documents

**[S01] Dennett Functional Concept.** Product vision, desktop/mobile, projects, voice, memory, server and capabilities.  
`00_Dennett_Functional_Concept.md`

**[S02] Dennett Specification Index.** Ownership, canonical state and document boundaries.  
`01_Dennett_Specification_Index_and_Shared_Contracts.md`

**[S03] Dennett Memory Fabric 1.2.** Memory, project packs, deletion, offline and retrieval.  
`10_Dennett_Memory_Fabric.md`

**[S04] Dennett Agentic Control Fabric 1.1.** Sessions, Tasks, Runs and pragmatic execution.  
`20_Dennett_Agentic_Control_Fabric.md`

**[S05] Dennett Trust Fabric.** Identity, grants, secrets, confirmations and incidents.  
`30_Dennett_Trust_Identity_Autonomy_and_Permissions.md`

**[S06] Dennett Voice Fabric.** Voice Session, ambient, turn-taking and handoff.  
`40_Dennett_Voice_and_Ambient_Interaction_Fabric.md`

**[S07] Dennett Capability Fabric.** Providers, skills, MCP, plugins, connectors and lifecycle.  
`41_Dennett_Capabilities_Providers_and_Integrations.md`

**[S08] Dennett Server Runtime.** Head, Node, sync, effects, backups and portability.  
`50_Dennett_Server_Runtime_Events_Sync_and_Portability.md`

**[S09] Dennett Desktop Business Logic.** Workbench, commands, states, accessibility and phases.  
`60_Dennett_Desktop_Application_Business_Logic.md`

**[S10] Dennett Mobile Business Logic.** Trusted remote, capture, interruption, offline and OS surfaces.  
...
`82_Dennett_Agent_Voice_Capability_and_Integration_Architecture.md`
