# Denet System Architecture and Runtime Topology
...
**Статус:** первый канонический архитектурный том. Документ определяет системную форму Denet и служит основанием для последующих томов данных, интеллектуального исполнения и клиентской реализации.  
**Каноническое имя:** `80_Denet_System_Architecture_and_Runtime_Topology.md`

Этот документ является самостоятельным. Чтобы понять выбранную архитектуру, читателю не требуется знать историю обсуждения проекта. При этом документ нормативно продолжает действующий комплект бизнес-логики Denet:
...
> **Из каких исполняемых частей должен состоять Denet, где они живут, как взаимодействуют, как переживают ошибки и обновления, как заменяются без переписывания всей системы и как организовать код так, чтобы длительная разработка людьми и агентами оставалась управляемой?**
...
Denet следует строить как **process-selective modular monolith** — модульный монолит с небольшим числом осмысленных исполняемых границ и архитектурой ports-and-adapters.
...
Третья: границы модулей определяются **capabilities и domain contracts**, а не названиями конкретных провайдеров. OpenAI, Anthropic, Playwright, Screenpipe, Tailscale, Restate или другой продукт являются адаптерами. Ни один из них не диктует внутреннюю модель Denet.

Четвёртая: одна и та же логическая часть может иметь два способа размещения. Например, Memory Core может работать внутри Head Runtime в минимальной установке либо через отдельный `denet-memoryd` в персональном серверном профиле. Оба варианта обязаны проходить один набор contract tests и вести себя одинаково на уровне бизнес-контракта.
...
**Tauri 2** используется как desktop application shell. Tauri уже разделяет привилегированный Rust Core и WebView-процесс, поддерживает capability-scoped IPC, external binaries, подписанные обновления, single-instance, autostart, системный tray и cross-platform bundling. Это хорошо соответствует требованию Denet: богатый desktop UX без переноса секретов и authoritative logic в JavaScript. [[S12]] [[S13]] [[S14]] [[S15]]
...
Desktop-приложение не является сервером Denet и не владеет долговечными процессами.

Принято разделение:

- `denet-desktop` — Tauri UI, окна, tray, local presentation state, визуальные overlays и OS-specific user interaction;
- `denet-node` — постоянный Rust-демон устройства, продолжающий работу после закрытия окна и даже полного выхода из UI;
- при local-only установке `denet-node` может одновременно размещать Head Runtime;
- при personal-server установке node соединяется с удалённым `denet-head`.

Tauri умеет упаковывать и запускать sidecar binaries, но sidecar как дочерний процесс UI недостаточен для постоянного фонового ядра: закрытие, update или авария desktop shell не должны завершать capture, offline queue, локальную модель, активную project operation либо долгий Run. Поэтому installer регистрирует `denet-node` через штатный механизм фонового пользовательского сервиса ОС. Tauri используется для установки, управления и диагностики демона, а не как его lifetime owner. [[S12]] [[S13]]
...
1. **`denet-memory-core`** — переиспользуемая библиотека с канонической логикой Event/Evidence/Claim/Current State, retrieval planning, scope, correction, deletion и project packs;
2. **`denet-memoryd`** — необязательный самостоятельный процесс, предоставляющий ту же логику через versioned service contract.
...
Поэтому в минимальном профиле Head использует `InProcessMemoryAdapter`, а в рекомендуемом персональном серверном профиле — `MemoryServiceAdapter` к локальному `denet-memoryd`. Оба адаптера реализуют один `MemoryPort` и проходят общий conformance suite.

Существующие продукты Mem0, Letta, Zep/Graphiti и Cognee подтверждают полезность отдельного memory layer и дают готовые идеи для integrations, graphs и retrieval. Однако ни один из них не покрывает одновременно доказательный Event Ledger Denet, bitemporality, exact deletion, portable project memory, influence policy, offline conflict preservation и человекочитаемые projections. Поэтому они рассматриваются как benchmark/reference/adapter, но не как каноническое ядро памяти. [[S31]] [[S32]] [[S33]] [[S34]]
...
Готовый продукт никогда не получает право стать источником истины бизнес-логики. Например, Screenpipe может захватывать и предварительно индексировать экран, но каноническая память Denet формируется через `Sensor Adapter` и Memory Fabric. Tailscale может дать сетевой маршрут, но не выдаёт Denet permissions. Provider-native computer-use выполняет действия, но Effect Claim и Trust Decision принадлежат Denet.
...
2. Можно ли в минимальной установке запустить Denet без Kubernetes, message broker, Tailscale и отдельной БД-сетки?
...
8. Можно ли использовать Memory Core отдельно от Denet, не выдавая внешнему потребителю прямой доступ к внутренней БД?
...
- действующие бизнес-логические документы Denet.
...
10. Tailscale/Headscale являются транспортом, а не identity/authorization source Denet.
...
Denet не вводит полноценный CQRS framework. Разделяется только смысл:
...
**Владелец Denet** — основной human principal, управляющий установкой, проектами, данными, автономностью и recovery.

**Другие люди** — собеседники в Telegram/email/встречах, участники проектов и источники внешних сообщений. Они не становятся владельцами Denet из-за того, что их речь или текст попали в систему.
...
**Optional Network Overlay** — Tailscale или Headscale-compatible control server, предоставляющий маршрутизацию, но не бизнес-права Denet.
...
Система Denet в этой диаграмме является одним продуктом независимо от количества процессов. Ни Tailscale, ни provider, ни memory backend не являются частью доменной власти Denet.
...
```mermaid
flowchart TB
    UI[denet-desktop / Tauri]
    NODE[denet-node + Head composition]
...
- UI можно закрыть; `denet-node` остаётся активным;
...
Назначение: основной постоянный режим Denet.
...
- Denet должен уметь работать в одной LAN без Tailscale;
- Tailscale account не равен Denet principal;
- remote provider/cloud integration всё равно использует обычные APIs;
- embedded `tsnet` официально является Go library, а core Denet выбран на Rust. Встраивание Go runtime только ради транспорта усложнило бы build и supervision. [[S20]]
...
`denet-node` не решает глобальные permissions самостоятельно. Он исполняет signed/current grants либо ограниченную emergency policy.
...
**Тип:** optional separate Rust process вокруг `denet-memory-core`.  
...
- `denet-node`;
- `denet-head`;
- `denet-memoryd` при отдельном размещении.
...
flowchart LR
    ...
    INP --> CORE[denet-memory-core]
    RPC --> MEMD[denet-memoryd]
    MEMD --> CORE2[denet-memory-core]
```
...
- не получать весь Denet Orchestrator и UI.

Это создаёт реальную самостоятельную ценность и повышает качество самого Denet: reusable boundary заставляет отделить memory semantics от случайных деталей приложения.
...
**Managed Run:** lightweight durable envelope в Denet store:
...
**Structured Automation:** использует `DurableExecutionPort`, реализация которого может быть Denet-native, Restate или Temporal.
...
Restate интересен для Denet тем, что предлагает durable execution, journals, deduplication и single-writer state в относительно компактном server runtime, включая Rust ecosystem. Он является сильным кандидатом для optional Structured Automation и durable object-like coordination. [[S11R]]
...
→ locate/start denet-node if absent
...
- собственное local storage не равно Denet Memory;
- main branch быстро меняется;
- resource claims нужно проверить на целевых устройствах;
- Denet нужны свои privacy, deletion и project linking rules.
...
FoundationDB показывает силу deterministic simulation: целый cluster моделируется в одном процессе, failures инъецируются массово, а deterministic execution делает редкий bug повторяемым. Denet не обязан копировать весь Flow runtime, но должен перенять принцип для control-plane semantics. [[S41]]

### 36.2. Denet Simulation Harness

`denet-testkit` предоставляет:
...
`denet-node`, OS integrations, project filesystem, local execution. Имеет сильные local capabilities, но global actions ограничены grants/Head.
...
sequenceDiagram
    ...
    participant UI as denet-desktop
    participant N as denet-node
    participant H as denet-head
...
- **ADR-004:** Persistent `denet-node` independent of UI.
...
- **ADR-P03:** Restate vs Temporal vs Denet-native structured automation.
...
- one installer and `denet doctor`;
...
`81_Denet_Data_Memory_Storage_Sync_and_Protocol_Architecture.md` должен выбрать и описать:
...
`82_Denet_Agent_Voice_Capability_and_Integration_Architecture.md` должен детализировать:
...
`83_Denet_Client_Operations_Testing_and_Implementation_Blueprint.md` должен определить:
...
Полная микросервисная архитектура предложила бы отдельные сервисы memory, events, inbox, radar, scheduler, projects, agents, capabilities, trust, artifacts, usage, notifications и search. Она дала бы независимое deployment, но цена для Denet чрезмерна:
...
Из микросервисной дисциплины Denet берёт владельцев данных, explicit contracts, автономные modules и outbox. Сетевое распределение добавляется позже и выборочно.
...
### 73.1. Роли Tauri Core и `denet-node`
...
`denet-node` отвечает за persistent device runtime:
...
### 73.3. Поведение кнопок Close, Exit и Stop Denet
...
External client создаёт own principal/scope and cannot query all Denet personal memory by default.
...
### 76.1. What Denet actually needs
...
Event-driven architecture can hide control flow. Denet uses direct calls for:
...
Provider tool call is proposal. Denet maps tool name to capability, resolves grant, executes through proper adapter and returns result. Provider cannot call arbitrary local function by string.
...
- Denet project record, path binding and lifecycle; repository/filesystem remains file authority.
...
6. Tailscale account state does not revoke Denet device identity.
...
6. Existing captured objects remain in Denet memory independent of Screenpipe DB.
...
Denet не требует actor framework для каждого object. Actor-like owner task полезен, когда существует:
...
- `denet-desktop` installers per OS/arch;
- `denet-node` binaries/services;
- `denet-head` binary/container/package;
- `denet-memoryd` optional binary/container;
...
A small `denet` CLI is part of architecture, even if ordinary users use UI.
...
- `Effect UNKNOWN` → “Сервис мог выполнить действие. Denet проверяет результат и не повторяет его.”
...
# Required tests
- cargo nextest run -p denet-node
- cargo test -p denet-node --test contract
- denet-sim scenario node-reconnect
- denet-sim scenario old-epoch-rejected
...
Screen/audio capture has CPU, RAM, disk, battery budgets per mode and dynamic degradation. Screenpipe claims event-driven resource reductions, but Denet validates on target hardware rather than adopting README numbers as guarantees. [[S39]]
...
# Часть XXXIV. Denet как конструктор: composition и замена частей

## 145. Статическое ядро и динамические компоненты

Архитектура-конструктор не означает, что абсолютно всё является plugin. Полностью динамическое ядро было бы трудно типизировать, тестировать и защищать. Denet делит систему на два слоя.
...
Native features exposed via typed extension namespaces. A provider without sessions can emulate handle through Denet context, but adapter declares lower continuity fidelity.
...
- `denet-node` contains Head host modules;
...
1. `denet-desktop` can be killed while Run continues.
2. `denet-node` can run without WebView.
...
**[S01] Denet Functional Concept.** Видение продукта, проекты, оркестратор, голос, память, sensors, интерфейсы и переносимость.  
`00_Denet_Functional_Concept.md`

**[S02] Denet Specification Index and Shared Contracts.** Ownership документов, общие контракты и источники истины.  
`01_Denet_Specification_Index_and_Shared_Contracts.md`

**[S03] Denet Memory Fabric 1.2.** Evidence/event memory, retrieval, project packs, deletion, offline и переносимость.  
`10_Denet_Memory_Fabric.md`

**[S04] Denet Pragmatic Agentic Control Fabric 1.1.** Single-agent-first, Task/Run, Managed Run, completion и provider adapters.  
`20_Denet_Agentic_Control_Fabric.md`

**[S05] Denet Trust Fabric.** Identity, permissions, grants, effects, secrets и workspace trust.  
`30_Denet_Trust_Identity_Autonomy_and_Permissions.md`

**[S06] Denet Voice and Ambient Fabric.** Voice session, fast/slow path, ambient edge и turn-taking.  
`40_Denet_Voice_and_Ambient_Interaction_Fabric.md`

**[S07] Denet Capability Fabric.** Providers, models, skills, MCP, plugins, connectors, computer-use и local runtimes.  
`41_Denet_Capabilities_Providers_and_Integrations.md`

**[S08] Denet Server Runtime.** Head, local-capable nodes, sync, effects, backup и failover.  
`50_Denet_Server_Runtime_Events_Sync_and_Portability.md`

**[S09] Denet Desktop Business Logic.** Adaptive Agent Workbench и desktop states.  
`60_Denet_Desktop_Application_Business_Logic.md`

**[S10] Denet Mobile Business Logic.** Interruptible Agent Remote и mobile/offline flows.  
`61_Denet_Mobile_Application_Business_Logic.md`

**[S11] Denet End-to-End and Pre-Architecture Completion.** Quality scenarios, gaps, architecture requirements и завершённые cross-domain contracts.  
`70_Denet_End_to_End_Validation_and_Architecture_Handoff.md`  
`../specifications/contracts/README.md`
...
«Server», «PC», «laptop» и «phone» описывают hardware; `Head`, `Node`, `Memory Service` и `client` описывают роли. Достаточно мощный ПК может быть обычным персональным сервером, если пользователь установил и включил такой профиль. В этом случае он запускает те же `denet-head`, optional `denet-memoryd`, object-store role и supervision contracts, что и выделенный сервер.
