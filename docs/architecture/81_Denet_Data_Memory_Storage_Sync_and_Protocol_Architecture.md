# Denet Data, Memory, Storage, Sync and Protocol Architecture
...
**Каноническое имя:** `81_Denet_Data_Memory_Storage_Sync_and_Protocol_Architecture.md`.

Этот документ является самостоятельным. Он предполагает только общее понимание Denet как персональной агентной операционной системы: у пользователя есть проекты и проектные агенты, главный оркестратор, долговременная память, голосовой и ambient-режимы, несколько устройств, подключаемые модели и инструменты, фоновые задачи и возможность работать локально, через персональный сервер или гибридно.

Том отвечает на вопрос:

> **Где физически живёт каждое состояние Denet, какие данные являются каноническими, как они записываются, ищутся, синхронизируются, удаляются, восстанавливаются и передаются между процессами и устройствами без превращения персональной системы в тяжёлую распределённую платформу?**

Он продолжает архитектурный том `80_Denet_System_Architecture_and_Runtime_Topology.md`, который выбрал process-selective modular monolith, один логический Head Runtime, локально способные Device Nodes, Rust-ядро, Tauri desktop, заменяемые адаптеры и необязательный `denet-memoryd`. Этот том не меняет эти решения, а делает их реализуемыми на уровне данных. [[S12]]
...
Для первой production-способной версии Denet принимается следующий физический baseline:
...
```text
PostgreSQL cluster
├── denet_control   # Head Runtime и operational modules
└── denet_memory    # Memory Core / memoryd
```
...
- `denet-memory-core` должен оставаться переиспользуемым вне полного Denet;
...
Так Denet сохраняет бизнес-смысл offline-команды и не смешивает техническую репликацию страниц БД с разрешениями, конфликтами и внешними эффектами.
...
> **Denet Data Architecture — это минимальная полиглотная система: PostgreSQL хранит серверную operational и доказательную истину, SQLite даёт локальность и offline, content-addressed object storage удерживает большие неизменяемые bytes, а все интеллектуальные индексы являются сменными проекциями. Синхронизация передаёт доменные операции, протоколы версионируются, а ни UI, ни provider, ни vector store не получают право стать скрытым источником истины.**
...
Couchbase Lite остаётся технически сильной альтернативой для готового mobile/edge sync, но сделал бы Denet зависимым от собственной document/revision/conflict модели Couchbase. Он может быть повторно рассмотрен, если custom sync окажется существенно дороже и менее надёжным, чем ожидается.
...
Принят domain operation protocol, потому что Denet должен синхронизировать не только документы, но и:
...
**Authority:** `denet_control` на Head.  
...
PostgreSQL выбран не потому, что «реляционная БД решает всё», а потому что Denet нуждается в сочетании:
...
```text
postgresql cluster
├── database: denet_control
│   ├── role: denet_control_owner
│   ├── role: denet_head_runtime
│   └── role: denet_control_readonly
└── database: denet_memory
    ├── role: denet_memory_owner
    ├── role: denet_memory_runtime
    └── role: denet_memory_readonly
```
...
Внутри `denet_control` таблицы распределены по schemas или жёстким префиксам владельцев:
...
UI и adapter processes не открывают файл для самостоятельной записи. Они обращаются к `denet-node` или embedded owner через application API.
...
Rust `object_store` crate является полезным кандидатом для унифицированной работы с local filesystem и cloud object stores, но Denet сохраняет собственные semantics Object ID, encryption, staging и deletion. [[S34]]
...
Протокол может использовать tus или S3 multipart adapter, но canonical semantics принадлежат Denet. [[S33]] [[S36]]
...
Путь вроде `C:\code\denet` не является global identity проекта.
...
- Denet DB хранит observations, links, sessions, decisions и desired operations.

Memory summary «файл содержит X» не переопределяет фактический файл.

### 10.3. File synchronization

Denet не пишет собственный universal file-sync engine в первой версии.
...
Denet Sync Coordinator знает strategy и состояние, но не выдаёт один file-sync backend за универсальный.
...
Denet-owned file generation использует:
...
- Denet-native project instructions;
- imports;
- precedence;
- path scopes;
- hashes/revisions;
...
Denet должен одинаково хорошо находить:
...
Qdrant добавляется, если на реалистичном Denet corpus одновременно выполняется одно из условий:
...
Qdrant collection всегда rebuildable из memory metadata/object sources. [[S41]] Свежие operational-исследования PostgreSQL/pgvector рассматриваются как дополнительное подтверждение единого baseline, но не заменяют собственные Denet-бенчмарки фильтрованного retrieval, rebuild и обслуживания. [[S49]]
...
Provider-native idempotency используется, если есть. Иначе Denet хранит own key, provider operation mapping и reconciliation strategy.
...
- Denet может предложить merge, но не прячет конфликт.
...
Tailscale/Headscale может предоставить network path, но ticket и authorization принадлежат Denet.
...
Поэтому Electric не заменяет Denet Sync Protocol. [[S46]]
...
```yaml
id: uuidv7
source: denet://head/runtime
spec_version: 1
subject: run/...
type: denet.run.completed
time: ...
data_schema: ...
data: ...
correlation_id: ...
...
Denet может map внешние события в CloudEvents format, но domain event не зависит от CloudEvents SDK.
...
`.denet/memory/` является рабочим именем до стабилизации публичного формата.

### 51.2. Manifest

```yaml
package_type: denet.project_memory
...
`README.md`, notes, decisions и research остаются читаемыми без Denet.
...
- caches;
- Device replicas;
- project pack exports managed by Denet;
- backup snapshots;
...
If user copied package outside Denet, Denet cannot guarantee erase. It records export and warns honestly.
...
- refuse activation;
- suggest compatible Denet/update.
...
Denet does **not** retain all ambient audio by default; most ring-buffer content is overwritten.
...
`82_Denet_Agent_Voice_Capability_and_Integration_Architecture.md` должен:
...
`83_Denet_Client_Operations_Testing_and_Implementation_Blueprint.md` должен:
...
**[S01] Denet Functional Concept.** Product vision, project agents, memory, server, devices, ambient capture and portability.  
`00_Denet_Functional_Concept.md`

**[S02] Denet Specification Index and Shared Contracts.** Ownership, sources of truth, common references and architecture handoff.  
`01_Denet_Specification_Index_and_Shared_Contracts.md`

**[S03] Denet Memory Fabric 1.2.** Evidence-first memory, bitemporality, selective semantic structure, retrieval, federation, deletion and portable project memory.  
`10_Denet_Memory_Fabric.md`

**[S04] Denet Pragmatic Agentic Control Fabric 1.1.** Tasks, Runs, checkpoints, single-agent-first execution and external-effect requirements.  
`20_Denet_Agentic_Control_Fabric.md`

**[S05] Denet Trust, Identity, Autonomy and Permissions.** Principals, grants, Secret Broker, prompt injection, external effects and recovery.  
`30_Denet_Trust_Identity_Autonomy_and_Permissions.md`

**[S06] Denet Capability Fabric.** Providers, local models, skills, MCP, connectors and backend lifecycle.  
`41_Denet_Capabilities_Providers_and_Integrations.md`

**[S07] Denet Server Runtime.** Head Runtime, offline logs, sync classes, effects, backup and failover.  
`50_Denet_Server_Runtime_Events_Sync_and_Portability.md`

**[S08] Denet Voice Fabric.** Committed turns, ambient sources, media pipeline and local edge.  
`40_Denet_Voice_and_Ambient_Interaction_Fabric.md`

**[S09] Denet Desktop and Mobile Business Logic.** Search, caches, states, devices, offline queue and user-visible recovery.  
`60_Denet_Desktop_Application_Business_Logic.md`, `61_Denet_Mobile_Application_Business_Logic.md`

**[S10] Denet End-to-End Validation.** Gap audit, quality scenarios and architecture requirements.  
`70_Denet_End_to_End_Validation_and_Architecture_Handoff.md`

**[S11] Denet Pre-Architecture Completion.** Sensory capture, communication, artifacts, packages, search, recovery and usage contracts.  
`../specifications/contracts/README.md`

**[S12] Denet System Architecture 80.** Process-selective modular monolith, Rust/Tauri, Head/Node/memoryd, ports/adapters, outbox and test topology.  
...
**[S49] PostgreSQL/pgvector operational and filtered-search studies.** Recent preprints and case studies support unified storage as a practical baseline while showing optimizer/filtering caveats; decisions still require Denet-specific benchmarks.  
