...
Owns: orchestrator-visible `launch`, `status`, `wait`, `send`, `cancel`, and `close` semantics for managed subagent work; bounded capability rules; owned value objects and enums for the managed task package; normalized result-shape rules; and invalid boundary cases.
...
- `worker` performs bounded implementation or execution work.
- `reviewer` checks a bounded result and may return reviewer-like `findings`.
- `explorer` gathers bounded context or options inside its declared read and write surface; it is not reviewer-like by default.
- `integrator` combines accepted outputs or prepares a bounded final handoff; it is not reviewer-like by default.
- `final_review` performs final review and is reviewer-like.
- Only `reviewer` and `final_review` are reviewer-like for `findings` and review-loop budget semantics.

## 3. Logical Methods

This contract uses six logical methods.
...
| `child_role` | `worker`, `reviewer`, `explorer`, `integrator`, or `final_review` | Managed child role. |
...
Status returns the durable managed child state known to Core.

Request fields:

| Field | Type / constraints | Meaning |
| --- | --- | --- |
| `subagent_id` | non-empty string | The child to inspect. |
...
| `run_id` | non-empty string | Parent-side managed run identifier. |
| `child_run_id` | non-empty string | Underlying child run identifier. |
| `child_role` | any allowed `child_role` value | Managed role used for the child. |
| `lineage` | closed `lineage` object | Parent/child linkage metadata. |
| `task_package` | closed managed task-package snapshot | Durable launch package and recorded control messages. |
| `state` | any allowed `state` value | Current durable managed state. |
| `terminal_result` | optional closed terminal result object | Present after terminal reconciliation. |
| `close_disposition` | optional close disposition | Parent disposition when recorded. |

Rules:

- `status` is a durable state projection, not a live runtime probe.
- `status` must not claim cross-process attachment or live child control.
...
- `cancel`: may contain an optional human-readable `reason`; it records parent cancellation intent and moves the managed state toward cancellation, but it must not claim live runtime cancellation delivery.
...
Cancel records parent cancellation intent for a managed child.
...
| --- | --- | --- |
| `subagent_id` | non-empty string | Child identifier. |
| `message_id` | optional non-empty string | Parent-generated id for deduplication and audit. |
...
| --- | --- | --- |
| `subagent_id` | non-empty string | Child identifier. |
| `cancel_status` | `cancelling`, `already_cancelling`, `already_terminal`, `already_closed`, or `rejected` | Whether cancellation state was newly recorded, already inapplicable, or rejected before state mutation. |
| `state` | any allowed `state` value | Current durable managed state after cancellation handling. |
| `outcome` | optional allowed terminal outcome | Present when a terminal result already exists. |
| `reason_code` | optional allowed `reason_code` | Present when a terminal result already has a reason, and required when `cancel_status = rejected`. |
| `runtime_cancellation_delivered` | literal `false` | Explicitly states that this contract does not claim live runtime cancellation delivery. |

Rules:

- `cancel` is equivalent to recording a bounded `cancel` control message through the managed state surface.
- For a non-terminal child, successful cancellation moves durable state to `cancelling` and records parent disposition as `cancelled_by_parent`.
- A rejected cancellation must return `cancel_status = rejected`, must include a product-level `reason_code`, and must not record cancellation intent, change child state, or claim live runtime cancellation delivery.
- A cancelling child still requires `wait` to observe terminal cancellation before `close` can release the boundary.
- `cancel` is idempotent for repeated cancellation of a child already in `cancelling`, `terminal`, or `closed` state.
- `cancel` is state-level cancellation only. It does not prove that the underlying runtime received or honored a cancellation signal.
...
| --- | --- | --- |
| `subagent_id` | non-empty string | Child identifier. |
| `child_role` | `worker`, `reviewer`, `explorer`, `integrator`, or `final_review` | Role used for this child. |
...
- `status` reports durable managed state only and does not probe live runtime state.
- `cancel` records state-level cancellation intent only and does not claim live runtime cancellation delivery.
...
- cancel requests or responses that claim live runtime cancellation delivery without a documented runtime capability;
...
Владеет: семантикой `launch`, `status`, `wait`, `send`, `cancel` и `close` для управляемой subagent-работы; правилами bounded capability; собственными value-objects и enum-ами для managed task package; нормализованной формой результата; и невалидными boundary-cases.
...
Допустимые значения `child_role`:
...
- `worker` выполняет bounded implementation или execution work.
- `reviewer` проверяет bounded result и может возвращать reviewer-like `findings`.
- `explorer` собирает bounded context или options внутри объявленного read/write surface; по умолчанию это не reviewer-like role.
- `integrator` объединяет accepted outputs или готовит bounded final handoff; по умолчанию это не reviewer-like role.
- `final_review` выполняет final review и является reviewer-like.
- Только `reviewer` и `final_review` являются reviewer-like для `findings` и review-loop budget semantics.

## 3. Logical Methods

Этот контракт использует шесть logical methods.
...
| `child_role` | `worker`, `reviewer`, `explorer`, `integrator` или `final_review` | Роль managed child-а. |
...
Status возвращает durable managed child state, известный Core.

Поля request:

| Поле | Тип / ограничения | Смысл |
| --- | --- | --- |
| `subagent_id` | непустая строка | Child, который нужно inspect. |

Поля response на status:

| Поле | Тип / ограничения | Смысл |
| --- | --- | --- |
| `subagent_id` | непустая строка | Идентификатор child-а. |
| `run_id` | непустая строка | Parent-side managed run identifier. |
| `child_run_id` | непустая строка | Underlying child run identifier. |
| `child_role` | любое допустимое значение `child_role` | Managed role, использованная для child-а. |
| `lineage` | закрытый объект `lineage` | Metadata связи parent/child. |
| `task_package` | закрытый snapshot managed task package | Durable launch package и recorded control messages. |
| `state` | любое допустимое значение `state` | Текущее durable managed state. |
| `terminal_result` | необязательный закрытый terminal result object | Присутствует после terminal reconciliation. |
| `close_disposition` | необязательный close disposition | Parent disposition, когда он записан. |

Правила:

- `status` является durable state projection, а не live runtime probe.
- `status` не должен заявлять cross-process attachment или live child control.
...
- `cancel`: может содержать необязательный human-readable `reason`; он записывает parent cancellation intent и переводит managed state к cancellation, но не должен заявлять live runtime cancellation delivery.
...
Cancel записывает parent cancellation intent для managed child.
...
| --- | --- | --- |
| `subagent_id` | непустая строка | Идентификатор child-а. |
| `message_id` | необязательная непустая строка | Parent-generated id для deduplication и audit. |
| `reason` | необязательная строка | Human-readable cancellation reason. |

Поля response на cancel:

| Поле | Тип / ограничения | Смысл |
| --- | --- | --- |
| `subagent_id` | непустая строка | Идентификатор child-а. |
| `cancel_status` | `cancelling`, `already_cancelling`, `already_terminal`, `already_closed` или `rejected` | Была ли cancellation state записана впервые, уже неприменима или отклонена до изменения state. |
| `state` | любое допустимое значение `state` | Текущее durable managed state после cancellation handling. |
| `outcome` | необязательный допустимый terminal outcome | Присутствует, когда terminal result уже существует. |
| `reason_code` | необязательный допустимый `reason_code` | Присутствует, когда terminal result уже имеет reason, и обязателен при `cancel_status = rejected`. |
| `runtime_cancellation_delivered` | literal `false` | Явно фиксирует, что контракт не заявляет live runtime cancellation delivery. |

Правила:

- `cancel` эквивалентен записи bounded `cancel` control message через managed state surface.
- Для non-terminal child успешная cancellation переводит durable state в `cancelling` и записывает parent disposition как `cancelled_by_parent`.
- Отклоненная cancellation должна возвращать `cancel_status = rejected`, должна включать product-level `reason_code` и не должна записывать cancellation intent, менять state child-а или заявлять live runtime cancellation delivery.
- Cancelling child все еще требует `wait`, чтобы увидеть terminal cancellation перед тем, как `close` сможет освободить boundary.
- `cancel` idempotent для повторной cancellation child-а в состоянии `cancelling`, `terminal` или `closed`.
- `cancel` является только state-level cancellation. Он не доказывает, что underlying runtime получил или исполнил cancellation signal.
...
| --- | --- | --- |
| `subagent_id` | непустая строка | Идентификатор child-а. |
| `child_role` | `worker`, `reviewer`, `explorer`, `integrator` или `final_review` | Роль, использованная для этого child-а. |
...
- `status` сообщает только durable managed state и не выполняет live runtime probe.
- `cancel` записывает только state-level cancellation intent и не заявляет live runtime cancellation delivery.
...
- cancel requests или responses, которые заявляют live runtime cancellation delivery без документированной runtime capability;
