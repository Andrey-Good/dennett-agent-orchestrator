...
| First managed-subagent layer | Core service and state cover worker/reviewer/final-review roles, launch/wait/send/close, findings, cancellation state, budgets, and sibling write-set conflict rejection at the implemented slice. | Not an operator-facing managed-subagent product surface and not broad live orchestration proof. |
...
| Stage 8: Managed Subagent Product Surface | Managed subagents are not yet public operator-facing orchestration. | User-visible create/send/wait/status/close/cancel flows, enforced ownership and write-scope policies, review/fix loop evidence, lineage/budget behavior, cancellation cleanup, and leaked-child checks. |
...
- Stage 8 must prove managed subagents as a product surface before operator-facing orchestration claims expand.
...
| First managed-subagent layer | Core service и state покрывают worker/reviewer/final-review roles, launch/wait/send/close, findings, cancellation state, budgets и sibling write-set conflict rejection в реализованном slice. | Не является operator-facing managed-subagent product surface и не является broad live orchestration proof. |
...
| Stage 8: Managed Subagent Product Surface | Managed subagents еще не являются public operator-facing orchestration. | User-visible create/send/wait/status/close/cancel flows, enforced ownership и write-scope policies, review/fix loop evidence, lineage/budget behavior, cancellation cleanup и leaked-child checks. |
...
- Stage 8 должен доказать managed subagents как product surface перед расширением operator-facing orchestration claims.
