...
| --- | --- | --- |
| Package/public registry evidence | `package.json` no longer has `private: true`, version is the prepared release candidate `0.1.0-rc.1`, registry ownership is not proven, and public registry install/upgrade/uninstall/rollback proof is absent. | Final public version/tag/release notes approval, registry ownership proof, public artifact proof, and lifecycle proof for the selected public package. |
...
| Stage 4: Release Engineering And Supply Chain | Stage 4 proves only the release-prep package and release-engineering foundation; no npm/public package, installer, container, signed artifact, provenance, or public artifact rollback readiness is proven. | Release-prep packaging boundaries, repository-local release gates, artifact inventory inputs, and reproducible-build expectations. Clean-environment install/deployment proof for selected distribution artifacts, provenance/signing/publication decisions, rollback/uninstall proof, and CI OS parity remain Stage 11 or later release-approval evidence. |
...
- Stage 4 owns the release-prep package foundation only; Stage 11 must prove each selected distribution artifact instead of inferring artifact readiness from repository gates.
