...
| Dependency and supply-chain risk | Package install executes code and depends on the JavaScript and Python ecosystems plus runtime/provider tooling. | Stage 4 must keep the release-prep package foundation and inventory guards; Stage 11 must own local tarball install/uninstall proof, upgrade/rollback harness limits, local SBOM validation, and provenance/signing deferrals. |
...
- Stage 4 records the selected release-prep package foundation and inventory controls; Stage 11 records local tarball install/uninstall proof, explicit two-tarball upgrade/rollback smoke, local SBOM validation, and unsigned/unattested deferrals.
