...
Current public-readiness package metadata is the prepared release candidate `0.1.0-rc.1` with the package-level `private: true` guard removed for release preparation. Historical `0.0.0` values and older `private: true` rows describe the package metadata at the recorded evidence commits only; they are not current package, publication, or public version approval claims.

As of the 2026-04-30 repository public-preview check, GitHub API reported `private: false`, `visibility: public`, and `origin/main` resolved to `3ddcb5e70a25969b492108c0cb33e695b87137ed`. That supports `repository-public-preview` only. Removing the package-level `private: true` guard supports release preparation only; `oss-v0.1-release` package-publication claims remain blocked until registry, ownership, approval, publish, and public install evidence exist.
