...
| Real-world release proof beyond local and mocked slices | [Phase 19 Real-World Proof And Release](../20-real-world-proof-and-release/README.md), [Release Scope Lock](../20-real-world-proof-and-release/release-scope-lock.md), [Operational Readiness](../11-hardening/operational-readiness.md), [Release Gates](../11-hardening/release-gates.md) | `not_started`: no new product code is claimed for Phase 19 proof and no full release-ready state is claimed from local/offline evidence alone | `partial`: current repository gates, Phase 18 local/offline integration evidence, TASK-287 live graph smoke, TASK-290 stress/regression evidence, TASK-291 local operational/recovery/cleanup evidence, TASK-292 bilingual documentation cleanup, and TASK-454 deterministic local Stage 8 stress/recovery/regression gate rerun support only the locked `local-cli-repository-readiness` target | `partial`: TASK-280 proved runtime discovery and direct local Mem0 provider operations; TASK-287 proved minimal live Codex graph execution; TASK-454 confirms deterministic local-only stress/recovery/regression proof, including explicit retry/resume completion and exactly-once final output after crash/reopen, not live provider stress; the final release decision is `defer` for the locked local CLI/repository target, not `release` | [Phase 19](../20-real-world-proof-and-release/README.md); included and deferred capabilities are locked by [Release Scope Lock](../20-real-world-proof-and-release/release-scope-lock.md); broader release claims remain deferred by the [release decision record](../20-real-world-proof-and-release/release-decision-record.md), including hosted/package/container rollback, durable external provider cleanup, external-provider reliability, live provider stress, production-scale load, and runtime-native memory consumption inside Codex graph execution |
...
<a id="russian"></a>
Маршрутная заметка Phase 19: финальное решение теперь `defer` для суженного local CLI/repository scope, а не `block` и не `release`. TASK-280 дал проходящие gates, доказательство runtime discovery и прямое доказательство локального Mem0 provider; TASK-287 добавил проходящий minimal live graph smoke; TASK-290, TASK-291 и TASK-292 поддерживают stress/regression, local operational/recovery/cleanup и bilingual documentation cleanup только для этого суженного scope; TASK-333 добавил узкий included proof for registered local Mem0 plus Codex prompt-rendered memory context, while TASK-334 superseded the initial default-gate failure with a passing `pnpm test` rerun. Текущие доказательства все еще не поддерживают полный release или broader claims: hosted/package/container rollback, durable external provider cleanup, external-provider reliability и broader runtime-memory behavior beyond the narrow Stage 2 proof остаются deferred в [release decision record](../20-real-world-proof-and-release/release-decision-record.md). Для обеих языковых секций честный статус: код `not_started`, тесты `partial`, live proof `partial`, финальное решение `defer`.
...
| Real-world release proof beyond local и mocked slices | [Phase 19 Real-World Proof And Release](../20-real-world-proof-and-release/README.md), [Release Scope Lock](../20-real-world-proof-and-release/release-scope-lock.md), [Operational Readiness](../11-hardening/operational-readiness.md), [Release Gates](../11-hardening/release-gates.md) | `not_started`: для Phase 19 proof не заявляется новый product code, и local/offline evidence само по себе не дает полного release-ready состояния | `partial`: текущие repository gates, Phase 18 local/offline integration evidence, TASK-287 live graph smoke, TASK-290 stress/regression evidence, TASK-291 local operational/recovery/cleanup evidence и TASK-292 bilingual documentation cleanup поддерживают только зафиксированную цель `local-cli-repository-readiness` | `partial`: TASK-280 доказал runtime discovery и direct local Mem0 provider operations; TASK-287 доказал minimal live Codex graph execution; финальное release decision теперь `defer` для зафиксированной local CLI/repository цели, а не `release` | [Phase 19](../20-real-world-proof-and-release/README.md); included и deferred capabilities зафиксированы в [Release Scope Lock](../20-real-world-proof-and-release/release-scope-lock.md); broader release claims остаются deferred в [release decision record](../20-real-world-proof-and-release/release-decision-record.md), включая hosted/package/container rollback, durable external provider cleanup, external-provider reliability и runtime-native memory consumption inside Codex graph execution |
