# Dennett Mobile Application Business Logic
...
**Каноническое имя:** `61_Dennett_Mobile_Application_Business_Logic.md`.
...
Мобильное приложение Dennett не является уменьшенной копией desktop-приложения. Его главная задача — дать пользователю возможность за несколько секунд:
...
- функциональную концепцию Dennett — продуктовые цели и роль телефона как пульта; [[S01]]
...
### 0.1. Dennett Mobile — Glance, Capture, Decide, Continue

Лучшее решение — не «полный Dennett на маленьком экране» и не один гигантский голосовой экран.

Мобильное приложение Dennett должно быть **Interruptible Agent Remote** с четырьмя основными циклами:
...
> **Dennett Mobile — это быстрый доверенный пульт персональной агентной системы: glanceable состояние, voice-first управление, capture-first ввод, decision-first Inbox и interruption-safe продолжение работы.**
...
Dennett Mobile использует, где платформа позволяет:
...
7. поделиться ссылкой из браузера в Dennett;
...
## 4. Entry Surfaces — Dennett до открытия приложения
...
Android launchers обычно показывают ограниченное число shortcuts, поэтому Dennett не должен пытаться вместить туда весь продукт. [[S12]]

### 4.3. Widgets

Dennett предлагает несколько widgets, каждый с configurable privacy mode.
...
Dennett использует notification только для:
...
Apple Shortcuts демонстрирует, что действия могут запускаться через Siri, Control Center, Action button, widget, Home Screen, Search, Apple Watch и Back Tap; Dennett должен публиковать небольшой набор стабильных App Intents вместо сотен узких shortcuts. [[S20]]
...
После выбора Dennett открывает компактный Share Capture Sheet:
...
Action Inbox — долговечная очередь ситуаций, где Dennett действительно нуждается в выборе пользователя или не имеет права безопасно продолжить сам.
...
4. **Recommendation** — что предлагает Dennett и с какой уверенностью.
...
ChatGPT Voice provides a practical example of continuing a conversation while other apps are used or the phone is locked; Dennett applies this through its own Voice/Trust/Server contracts. [[S24]]
...
Before delivering a noncritical notification Dennett may consider:
...
- `Project Dennett: agent finished, tests passed, one decision needed`;
...
- Full;
- Project Alias only;
- Generic `Dennett needs a decision`;
- Count only;
- Hidden.
...
- “Когда появится важная AI-новость по экранной памяти, свяжи её с Dennett”.
...
Мобильный интерфейс Dennett часто работает поверх распределённого и частично доступного состояния:
...
Приложение установлено, но не связано с Dennett.

Primary actions:

- `Pair Existing Dennett`;
...
Mobile Dennett не должен ожидать, что пользователь завершит текущую мысль или экран за один непрерывный заход.
...
Исследования мобильного чтения показывают, что короткие review после interruptions нравятся пользователям, а previews предстоящего содержания способны улучшать понимание. [[S25]] Поэтому Dennett использует оба элемента: «что было» и «что дальше».
...
Перед открытием notification Dennett сохраняет:
...
Research по adaptive handedness подтверждает полезность такого направления, но Dennett не должен автоматически угадывать руку ценой нестабильного layout. [[S31]]
...
Car mode не даёт полный доступ к Dennett.
...
Dennett Mobile проектируется по принципам WCAG 2.2 и нативных accessibility APIs платформ. WCAG 2.2 отдельно включает требования к target size, dragging alternatives, accessible authentication, focus visibility, timing и status messages. [[S32]]
...
- запоминать отдельный Dennett challenge;
...
Пользователь может сразу убрать телефон. Позже Dennett:
...
> **Dennett Mobile — это interruption-safe trusted remote персональной агентной системы. Он оптимизирует не время, проведённое в приложении, а время до безопасного полезного результата: показывает только существенное, сохраняет любой capture до его организации, даёт быстро принять решение, позволяет голосом управлять Dennett, честно работает offline, восстанавливает контекст после прерывания и без трения передаёт сложную работу подходящему устройству.**
...
**[S01] Dennett Functional Concept.** Роль телефона как пульта, voice, capture, projects, Action Inbox, Radar, memory и offline emergency mode.  
`00_Dennett_Functional_Concept.md`

**[S02] Dennett Specification Index and Shared Contracts.** Ownership документов, canonical state и граница mobile UI.  
`01_Dennett_Specification_Index_and_Shared_Contracts.md`

**[S03] Dennett Memory Fabric 1.2.** Offline event log, capture, memory search, correction, deletion и project memory.  
`10_Dennett_Memory_Fabric.md`

**[S04] Dennett Pragmatic Agentic Control Fabric 1.1.** Project Session, Work Item, Task, Run, direct/managed execution, interruptions and completion.  
`20_Dennett_Agentic_Control_Fabric.md`

**[S05] Dennett Trust, Identity, Autonomy and Permissions.** Trusted mobile, biometric step-up, grants, external effects, voice identity и Emergency Stop.  
`30_Dennett_Trust_Identity_Autonomy_and_Permissions.md`

**[S06] Dennett Voice and Ambient Interaction Fabric.** Voice Session, PTT/live/meeting, interruption, ambient, device handoff и UI requirements.  
`40_Dennett_Voice_and_Ambient_Interaction_Fabric.md`

**[S07] Dennett Capabilities, Providers and Integrations.** Providers, skills, MCP, connectors, mobile computer-use backends и lifecycle.  
`41_Dennett_Capabilities_Providers_and_Integrations.md`

**[S08] Dennett Server Runtime, Events, Sync and Portability.** Head Runtime, devices, Action Inbox/Radar authority, offline queue, sync, backup и effect reconciliation.  
`50_Dennett_Server_Runtime_Events_Sync_and_Portability.md`

**[S09] Dennett Desktop Application Business Logic.** Adaptive Agent Workbench, Resume Strip, Quick Capture, Inbox, Radar, Handoff и shared command semantics.  
...
9. **High-impact mobile products focus on triage/review rather than complete desktop parity** → Mobile Dennett acts as trusted remote, not mini-IDE. [[S21]]
