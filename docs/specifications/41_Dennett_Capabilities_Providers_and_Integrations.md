# Dennett Capabilities, Providers and Integrations
...
> Related: [30_Dennett_Trust_Identity_Autonomy_and_Permissions.md](./30_Dennett_Trust_Identity_Autonomy_and_Permissions.md).
...
**Каноническое имя:** `41_Dennett_Capabilities_Providers_and_Integrations.md`.

Документ продолжает и не заменяет:

- `00_Dennett_Functional_Concept.md`;
- `01_Dennett_Specification_Index_and_Shared_Contracts.md`;
- `10_Dennett_Memory_Fabric.md` в актуальной версии 1.2;
- `20_Dennett_Agentic_Control_Fabric.md` в актуальной версии 1.1;
- `30_Dennett_Trust_Identity_Autonomy_and_Permissions.md`.

Он определяет **абстрактную бизнес-логику всей системы возможностей Dennett**:

- откуда возможности берутся;
- как Dennett узнаёт, что они существуют;
...
- как Dennett использует нативные возможности разных провайдеров и локальных runtimes;
...
Dennett должен иметь не каталог ссылок и не один плоский список установленных инструментов, а **Federated Capability Fabric** — живую систему, которая соединяет:
...
11. capabilities, созданные самим Dennett из опыта;
...
> **Dennett Capability Fabric — это федеративная, origin-aware и project-aware система возможностей, которая умеет находить, приобретать, сравнивать, безопасно подключать, минимально активировать и постепенно улучшать capabilities, не превращая каждое действие в бюрократию и не привязывая Dennett к одному поставщику.**
...
Пользовательское действие является достаточным основанием **немедленно добавить capability в коллекцию**. Dennett не запускает обязательный utility review, не сравнивает её со всей библиотекой и не пытается заменить выбор пользователя.
...
- Dennett не переписывает её автоматически;
...
### Capability возникла из опыта Dennett
...
В последнем случае Dennett создаёт **Capability Delta Proposal**:
...
1. **Registry** — всё, о чём Dennett знает, включая неустановленные кандидаты.
2. **Collection** — capabilities, которые пользователь или Dennett приобрели и могут повторно использовать.
...
- изменение Dennett-managed skills;
...
Dennett ничего не устанавливает и не закрепляет без явной команды. Может показывать найденное.

### Suggest

Dennett ищет, сравнивает и предлагает. Это рекомендуемый default для глобальной коллекции.

### Project-Autonomous

В доверенном проекте Dennett может добавлять и тестировать project-local candidates в пределах бюджета и Trust policy, не продвигая их глобально.

### Curated-Autonomous

Dennett может автоматически обновлять Dennett-managed capabilities и продвигать проверенные candidates по установленным acceptance rules. User-owned assets остаются защищёнными от автоматического переписывания.
...
Официальная документация подтверждает наличие функции, но не доказывает её качество. Benchmark показывает результат в конкретных условиях, но не превращается в универсальную истину Dennett.
...
- не привязывает Dennett к одному marketplace.
...
Без ownership upstream update может затереть ручные правки пользователя, а Dennett-generated update — изменить imported skill. Нужны managed, user-owned и forked варианты.
...
6. User-owned capability не переписывается Dennett автоматически.
7. Dennett-managed capability всегда versioned и rollbackable.
8. Provider-managed component обновляется по нативным правилам provider, но Dennett хранит observed diff.
...
19. Public benchmark является prior, а реальные Dennett outcomes — локальным evidence.
...
Откуда Dennett узнал о capability:
...
Факт приобретения capability данной установкой Dennett.
...
capability_installation:
  ...
  installation_mode: user_manual | project_import | provider_managed | dennett_managed | external_reference
  ownership: user | dennett | provider | project | shared
...
Dennett пытается понять, являются ли два результата одной capability:
...
Provider marketplace/update semantics сохраняются; Dennett фиксирует components и changes, но не пытается управлять package как обычным local folder, если provider этого не допускает.
...
OpenAI, Agent Skills и Hermes используют progressive disclosure: сначала краткая metadata, затем полный `SKILL.md` или reference по необходимости. Это принимается как базовый паттерн Dennett. [[S06]] [[S07]]
...
- Dennett не меняет содержимое автоматически;
- update показывается как proposal;
- автоматическая адаптация создаёт fork;
- пользователь может передать ownership Dennett;
- manual overwrite всегда возможен с diff/backup.

## 8.2. Dennett-managed

Создано Dennett или явно передано под управление.
...
Dennett строит candidate neighborhood по:
...
- Dennett не устанавливает B как default;
...
При создании проекта Dennett не прикрепляет десятки generic skills «на всякий случай».
...
- Dennett повышает её project prior;
...
Receiving Dennett показывает логически:
...
Public benchmark задаёт initial prior. Реальный опыт Dennett постепенно уточняет его для:
...
Hermes использует похожие triggers для agent-managed skills и отделяет короткую memory от длинной procedural memory. Dennett принимает паттерн, но делает project-local candidate default и не обязан свободно переписывать user-owned library. [[S08]]
...
SkillFoundry показывает практическую пользу циклов expand/repair/merge/prune для domain skill libraries; Dennett использует эту идею выборочно, а не как постоянный глобальный процесс. [[S09]]
...
Manual connection пользователя регистрируется немедленно. Dennett не обязан сначала оценивать «лучший ли это provider».

## 12.3. Multiple accounts и deployments

Connection имеет identity/account/workspace/region. Dennett не смешивает:
...
Для task classes Dennett накапливает:
...
Dennett normalization не должна превращать first-class runtime в простой `chat.completions` loop.
...
- Dennett не требует benchmark до добавления;
...
- Dennett не включает `trust_remote_code`-подобное поведение молча.
...
Dennett не держит все модели загруженными. Warm policy зависит от частоты, startup time, VRAM и active Runs.
...
Hermes реализует bundles и conditional activation; Dennett принимает идею, но выбирает bundle только при повторной совместной пользе. [[S08]]
...
- Dennett предлагает минимальный набор;
...
Hermes, например, сохраняет user-modified bundled skills при sync/remove и поддерживает inspect/install/check/update/audit/reset; этот практический pattern полезен для ownership semantics Dennett. [[S08]]

## 14.17. Skill mutation policy

- `patch` предпочтительнее полной перезаписи;
- user-owned — только proposal/fork;
- Dennett-managed project skill — auto patch по policy;
...
Dennett может включить компоненты частично. Установка server не означает exposure всех tools.
...
Dennett может создать semantic capability relation между ними и выбрать preferred implementation.
...
MCP security best practices требуют предотвращать token passthrough, confused deputy, SSRF, session hijacking и чрезмерные scopes. Dennett adapter должен сохранять эти boundary. [[S13]]
...
Dennett регистрирует package и components отдельно.

## 16.2. Native semantics

Claude marketplace, OpenAI/Codex plugins, Gemini extensions и IDE extensions имеют разные manifests, update channels и scopes. Dennett не flatten-ит их в один урезанный format.
...
Если package содержит полезный skill и нежелательный hook, Dennett должен по возможности:
...
Dennett выбирает самый структурированный достаточный слой:
...
Первая версия Dennett должна поддерживать готовые решения:
...
3. Dennett извлекает metadata и scripts list.
...
7. Dennett не редактирует skill автоматически.
...
5. Dennett не устанавливает новый как default.
...
3. Dennett может позже показать measured regression, но не снимает pin сам в Manual profile.
...
2. Dennett проверяет, не является ли это memory note/instruction/script.
...
3. Dennett предлагает successors/fallback.
...
1. Dennett читает capability requirements/references.
...
Для Dennett это кандидат first-class server-side agent runtime, особенно если пользователь использует Mistral models или managed connectors.
...
Он является кандидатом для Microsoft/Azure-centric installation, но Dennett не должен зависеть от него глобально.
...
Преимущество — быстрый доступ к сотням моделей и routing. Недостаток — дополнительный слой provider semantics и privacy. Для Dennett это first-class aggregator connection, но не замена direct adapters.

### 12.9. OpenClaw и Hermes как внешние runtime/reference systems

Они рассматриваются отдельно ниже. Dennett может:
...
Этот раздел является **snapshot на 11 июля 2026 года**. Перед реализацией или выбором модели Dennett проверяет live docs/API.
...
Полезен как search/research-oriented API provider и model endpoint. Dennett должен отличать provider search result от собственного Research workflow.
...
Рекомендация: отдельный Yandex Cloud connection с IAM/region/quota, но нативный agent adapter создавать только если Agent Atelier или managed runtime реально нужен Dennett.
...
Правило: отсутствие имени в этом документе не требует изменения ядра Dennett. Достаточно нового Provider Definition и adapter configuration, если API совместим.
...
Правило: Dennett предпочитает официальный publisher/revision, но не считает model безопасной только из-за популярности.
...
Подходит как flexible agentic browser backend; сравнивать с Skyvern и structured Playwright на реальных Dennett cases.

### 23.6. Собственный browser backend

Dennett не пишет браузерный движок. Возможная собственная часть:
...
Dennett поддерживает несколько provider-native adapters, а не выбирает один навсегда.
...
Dennett должен использовать, где возможно:
...
- connection to remote Dennett nodes;
...
Список является dynamic. Dennett не должен first-class реализовывать каждого provider до появления задачи.
...
Для локальных generative media workflows Dennett может подключать ComfyUI через API/workflow files вместо написания собственного image pipeline.
...
Может быть provider-built-in или Dennett Memory Fabric.

Provider file search не заменяет Dennett memory, потому что:
...
Dennett не нуждается в отдельном MCP, если project runtime уже имеет native shell. MCP/connector оправдан для remote environment или standardized external service.
...
### 28.5. Provider-managed connectors vs Dennett connectors
...
Dennett-native connector предпочтителен для core personal services, если нужен cross-provider use и единая policy.
...
Один control plane для sessions, channels, tools и events хорошо соответствует Dennett Server Runtime.
...
Команды onboarding/doctor снижают сложность настройки providers, channels и skills. Dennett следует иметь:
...
Отдельные Windows/macOS/mobile nodes, подключённые к Gateway, соответствуют модели Dennett server + device capabilities.

#### Model failover

OpenClaw поддерживает auth profiles/model fallbacks. Dennett заимствует идею, но применяет собственные privacy/trust/provider-lock rules.
...
OpenClaw README описывает full host access для main session как default personal mode. Dennett не должен автоматически копировать это для всех contexts.

В Dennett пользователь может выдать широкий Trusted-Elevated grant, но boundary остаётся явной и scoped.
...
- не встраивать весь runtime внутрь Dennett.
...
Повторяющийся опыт может создать candidate skill, но Dennett добавляет eval, versioning и rollback из своих документов.

#### Channel continuity

Одна система может продолжать session через CLI/Telegram/Discord и другие surfaces. Dennett реализует это через server canonical state, а не через копирование Hermes.
...
Ниже перечислены sources, поддерживающие устойчивые решения и датированный каталог. Научные работы 2026 года часто являются preprints; они используются как design evidence и должны подтверждаться собственными Dennett-specific evals.
...
12. Поддерживать provider-managed и Dennett-managed ownership.
