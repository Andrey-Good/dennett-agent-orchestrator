# Denet Pragmatic Agentic Control Fabric
...
> Related: [10_Denet_Memory_Fabric.md](./10_Denet_Memory_Fabric.md).
...
**Статус:** текущий исследовательский baseline бизнес-логики агентов. Версия 1.1 заменяет `Denet_agent_orchestration_logic_v1.0_agentic_control_fabric_2026.md` как рекомендуемую основу, сохраняя его сильные решения и отменяя избыточную обязательную формализацию.
...
Он не выбирает конкретный agent SDK, workflow engine, язык программирования, очередь или СУБД. Он определяет, **когда Denet должен просто дать сильному агенту работать, когда требуется лёгкий управляющий слой, когда оправдан subagent или reviewer и когда действительно нужна долговечная автоматизация**.
...
Несколько процентов выигрыша не являются улучшением, если они требуют кратно больше времени, токенов, provider limits и пользовательского ожидания. Denet должен оптимизировать не максимальный benchmark score, а **стоимость успешного и пригодного результата**.
...
Denet использует лестницу минимально достаточной оркестрации.
...
> **Denet всегда начинает с самого простого уровня, способного надёжно решить задачу, и повышает уровень только при наблюдаемой необходимости.**
...
> **Denet Pragmatic Agentic Control Fabric = strong-agent-first execution + lightweight durable control when needed + selective multi-agent delegation + optional structured automation + user-configurable oversight + evidence-based completion + cost-aware evaluation.**
...
Решения Denet принимаются по собственным ablations и реальным project trajectories.
...
Дополнительные состояния допускаются только для конкретного runtime или внешнего protocol adapter. Например, A2A использует submitted, working, input-required, auth-required и terminal states. На внутренней модели Denet они отображаются без необходимости копировать весь protocol во все микрозадачи. [[S10]]
...
Denet предпочитает provider-native runtime, если он уже даёт:
...
Если Claude Code умеет нативно вести session или Codex эффективно управляет tool calls, Denet не заставляет его имитировать универсальный `NextStep` JSON.
...
При старте Denet фиксирует assignment, context manifest, provider session, budget и owner. При завершении сохраняет result, artifacts, evidence и stop reason.
...
Если procedure запускается в фоне, Denet оборачивает её в Managed Run с checkpoints, budget и cancel. Стратегия остаётся у агента.
...
Bun сначала подготовил porting guide и trial на трёх файлах, затем масштабировал; однако десятки reviewers и workers оправданы размером million-line migration, а не являются default Denet. [[S09]]
...
Если качество нельзя объективно проверить, Denet не должен строить огромный completion contract. Он возвращает ограничения, evidence и uncertainty.
...
Denet оставляет её в project session. Task и workflow не создаются.
...
Denet минимизирует subagents и review, но сохраняет hard safety и external-effect confirmation.
...
**[S01] Denet functional concept.** Пользовательский project chat, оркестратор, агенты, workflow, память и регулируемая автономность.  
`00_Denet_Functional_Concept.md`

**[S02] Denet Memory Fabric 1.2.** Context Manifest, Working Memory, evidence, Causal Trace и federated spaces.  
`10_Denet_Memory_Fabric.md`
