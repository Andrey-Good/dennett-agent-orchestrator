# Dennett Trust, Identity, Autonomy and Permissions
...
> Related: [20_Dennett_Agentic_Control_Fabric.md](./20_Dennett_Agentic_Control_Fabric.md).
...
**Область:** Dennett как персональная агентная операционная система одного основного владельца с подключаемыми устройствами, агентами, проектами, провайдерами и внешними сервисами.
...
Dennett не должен строить безопасность на надежде, что сильная модель всегда правильно распознает опасность, prompt injection, чужой голос или пределы пользовательского намерения.
...
Безопасность Dennett должна различать пять вещей, которые часто ошибочно смешивают:
...
4. **Autonomy:** может ли Dennett выполнить разрешённое действие без нового вмешательства пользователя?
...
> **Dennett Trust Fabric — это вне-модельная, task-scoped, эффект-ориентированная и настраиваемая система доверия, в которой сильный агент сохраняет свободу стратегии, а runtime гарантирует границы доступа, аутентификацию, секреты, подтверждения и проверяемые последствия.**
...
- Агентная стратегия, Task, Work Item, Run и delegation — `20_Dennett_Agentic_Control_Fabric.md`.
- Evidence, trust domain памяти, deletion graph и memory influence — `10_Dennett_Memory_Fabric.md`.
- Список providers, tool manifests, installation lifecycle и connector adapters — `41_Dennett_Capabilities_Providers_and_Integrations.md`.
- Физическое исполнение sandbox, scheduler, sync и device gateway — `50_Dennett_Server_Runtime_Events_Sync_and_Portability.md`.
- Voice UX и распознавание turns — `40_Dennett_Voice_and_Ambient_Interaction_Fabric.md`.
...
Если Dennett позже станет многопользовательской командной платформой, модель principal ownership и policy engine может быть расширена без изменения базовых инвариантов.
...
Поэтому Dennett принимает не конкретный paper как догму, а повторяющийся системный вывод:
...
Dennett не должен обещать абсолютную защиту от:
...
Principal — сущность, чьи права, данные или воля представлены в Dennett.

Базовые principal types:

- `owner_user` — основной владелец Dennett;
...
Dennett может безопасно отвечать на общий вопрос или помогать человеку, если:
...
Каждое устройство Dennett получает устойчивую device identity, основанную на локально созданной ключевой паре.
...
Dennett не должен требовать максимальную аутентификацию для каждого действия. Вместо этого session имеет текущий **Assurance Level**, который может повышаться через step-up.
...
Dennett не хранит сырые biometric templates, если это не требуется платформой; предпочтительно использовать системный authenticator и получать только результат user verification. NIST также рассматривает biometrics как часть аутентификации, а не самостоятельный secret/authenticator. [[S01]]
...
Современные voice cloning системы способны обходить speaker verification по небольшим образцам; anti-spoofing плохо переносится между методами синтеза. Поэтому Dennett не должен строить security boundary на одной метрике сходства голоса. [[S17]]
...
- активный ли это явный разговор с Dennett;
- continuity turns;
- command risk;
...
Dennett не ставит медицинский диагноз «пьян», «неадекватен» или «болен».
...
Для персонального Dennett роли вроде `Developer`, `Operator`, `Reviewer`, `FinanceAdmin` быстро станут грубыми и неудобными. Один project agent сегодня редактирует CSS, завтра читает статью, а послезавтра запускает тест.
...
Для обычной задачи Dennett пытается автоматически вывести минимально достаточный scope:
...
Dennett не заставляет пользователя перечислять каждый path и domain.
...
Если Claude Code, Codex, ОС или connector уже обеспечивают sandbox/approval, Dennett не обязан дублировать каждую проверку.

Но Dennett сохраняет собственную верхнеуровневую policy:
...
Provider adapter переводит Dennett grant в нативные механизмы настолько точно, насколько возможно, и сообщает о потерях семантики.
...
Если Dennett часто спрашивает одно и то же, система должна предложить:
...
Dennett применяет тот же общий принцип, но не обязан копировать интерфейс VS Code.
...
Но доверие не должно быть жёстко привязано к каждому commit: это заставило бы подтверждать обычную работу после каждого pull. Вместо этого Dennett отслеживает **trust-sensitive diffs**.
...
При открытии своего существующего проекта Dennett может предложить:
...
Для клонированного незнакомого проекта default — Restricted. Dennett может в фоне подготовить краткий trust preview:
...
Проверки путей, tools и permissions на уровне Dennett/provider.
...
- Dennett-native project instruction;
...
Dennett не обязан копировать конкретный framework, но принимает идею **узкого проверяемого контракта текущего намерения**.
...
Dennett принимает упрощённый вариант:
...
Dennett не обязан реализовывать полный язык информационных потоков первой версии, но должен сохранять тот же принцип.

## 15.7. Safe continuation вместо тотального отказа

Prompt injection не должна автоматически останавливать полезную задачу.

Если malicious fragment найден в веб-странице, Dennett может:
...
В Dennett глубокая causal re-execution не является обязательным fast path. Она может применяться только при подозрительном high-value run или в security analysis mode.
...
- Dennett показывает краткий diff;
...
- server не получает Dennett-wide secret;
...
- не выдавать более широкое Dennett grant, чем provider реально поддерживает;
- отображать пользователю фактический scope;
- фиксировать внешний actor/session;
- маппить provider receipts в Dennett Effect Receipt;
...
Dennett сохраняет declaration, но доверие строится также на:
...
Dennett не обязан зависеть от Vault или GitHub; он принимает сам принцип.
...
Dennett может обнаруживать secrets в:
...
Внутри одной личной установки Dennett не требуется строить PKI для каждого десятисекундного subagent.
...
Если нет preauthorization или срочности, Dennett:
...
Dennett может redaction, заменить attachment ссылкой, спросить пользователя или отказаться.
...
## 20.11. Другой человек обращается к Dennett

Dennett может помочь безопасным ответом без раскрытия личного контекста, но другой человек не становится владельцем системы.
...
При срабатывании Dennett повторно проверяет:
...
В обычном случае Dennett показывает помощь, уведомляет доверенное лицо по preauthorization или просит подтверждение.
...
Step-up выполняется через trusted Dennett app/device с transaction details.
...
Dennett может заметить признаки:
...
Если prompts повторяются, Dennett предлагает узкий pattern. Он не расширяет права молча.
...
Receipt не обязан быть криптографически подписан в первой личной версии Dennett. Tamper-evident цепочки и signatures могут стать optional advanced mode для shared/team или external audit.
...
Если status unknown, Dennett должен сказать:
...
- что Dennett сделал сегодня от моего имени;
...
- почему Dennett спросил подтверждение;
...
Это лучше, чем либо полностью остановить Dennett, либо открыть всё.
...
- remote wipe Dennett encrypted state, если поддерживается;
...
Если provider не поддерживает idempotency, Dennett рассматривает call как at-most-once candidate и не повторяет автоматически после ambiguous failure.
...
2. Dennett читает структуру, instructions и manifest как untrusted data.
...
- effect E4;
- context unusual;
- Dennett предлагает archive/preview;
...
- Dennett показывает, какой service/scope нужен;
...
- владелец открывает Dennett на другом trusted device;
- revoke phone;
...
## 31.3. Dennett-specific test corpus
...
Отклонено: убивает основную полезность Dennett.
...
Ниже источники используются как evidence и практические ориентиры, а не как набор обязательных технологий. Preprints 2026 года требуют собственной проверки Dennett-specific eval.
