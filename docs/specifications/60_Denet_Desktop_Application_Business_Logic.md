# Denet Desktop Application Business Logic
...
**Каноническое имя:** `60_Denet_Desktop_Application_Business_Logic.md`.
...
### 0.1. Desktop Denet — Adaptive Agent Workbench
...
Desktop Denet должен быть **Adaptive Agent Workbench** — постоянной рабочей оболочкой, где:
...
> **Denet Desktop — это настраиваемый agent-first workbench: один глобальный shell, несколько контекстных рабочих пространств, постоянная видимость существенного состояния и минимальное количество прерывающих пользователя решений.**
...
VS Code показывает практическую ценность layout из editor area, primary и secondary sidebars, activity bar, panel и status bar, включая восстановление расположения после перезапуска, split views и floating windows. Denet использует этот паттерн, но заменяет code-first центр на контекстно меняющийся agent workbench. [[S09]] [[S10]]
...
- требует изучить внутреннюю архитектуру Denet до первого полезного действия;
...
Denet may include text/Markdown/code preview and lightweight editing, but is not required to replace a full IDE.
...
Action Inbox — не список уведомлений и не backlog. Это долговечная очередь ситуаций, где Denet действительно не должен или не может безопасно продолжить без решения пользователя.
...
> «Что сейчас происходит в Denet и где может понадобиться моё вмешательство?»
...
- Ownership: user/Denet/provider/project;
...
Denet compiles a readable summary, not necessarily a rigid workflow graph.
...
Denet may suggest destination, but user can override. Saving must not wait for deep analysis; semantic processing can occur later.
...
`denet://...` links may open app. External source is untrusted; link cannot directly execute consequential command without user-visible confirmation/policy.
...
When returning to a project/session after meaningful absence, Denet provides a compact delta:
...
After failure, Denet offers a recovery path instead of dumping logs:
...
Instead of many background completion notifications, Denet can group them into a digest:
...
`Ask Denet about this screen` attaches current UI location and selected object to a help question. It does not grant agent permission to click automatically.
...
**[S01] Denet Functional Concept.** Product vision, desktop as cockpit, direct project chats, Action Inbox, Radar, voice, memory and capabilities.  
`00_Denet_Functional_Concept.md`

**[S02] Denet Specification Index and Shared Contracts.** Ownership, sources of truth, common references and UI boundary.  
`01_Denet_Specification_Index_and_Shared_Contracts.md`

**[S03] Denet Memory Fabric 1.2.** Memory objects, context, evidence, correction, deletion, project memory and offline behavior.  
`10_Denet_Memory_Fabric.md`

**[S04] Denet Pragmatic Agentic Control Fabric 1.1.** Project sessions, single-agent-first execution, Task/Run, background work, review and completion.  
`20_Denet_Agentic_Control_Fabric.md`

**[S05] Denet Trust, Identity, Autonomy and Permissions.** Authentication, grants, approvals, workspace trust, external effects and emergency controls.  
`30_Denet_Trust_Identity_Autonomy_and_Permissions.md`

**[S06] Denet Voice and Ambient Interaction Fabric.** Voice Session, overlay states, interruption, ambient and multi-device voice.  
`40_Denet_Voice_and_Ambient_Interaction_Fabric.md`

**[S07] Denet Capabilities, Providers and Integrations.** Capability lifecycle, project sets, providers, models, skills, MCP, connectors and UI handoff.  
`41_Denet_Capabilities_Providers_and_Integrations.md`

**[S08] Denet Server Runtime, Events, Sync and Portability.** Head runtime, canonical Action Inbox/Radar state, devices, offline, sync, backup and recovery.  
