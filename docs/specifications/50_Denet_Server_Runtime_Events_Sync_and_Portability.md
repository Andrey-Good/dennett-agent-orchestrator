# Denet Server Runtime, Events, Sync and Portability
...
> Related: [40_Denet_Voice_and_Ambient_Interaction_Fabric.md](./40_Denet_Voice_and_Ambient_Interaction_Fabric.md) · [41_Denet_Capabilities_Providers_and_Integrations.md](./41_Denet_Capabilities_Providers_and_Integrations.md).
...
**Каноническое имя:** `50_Denet_Server_Runtime_Events_Sync_and_Portability.md`.
...
### 0.1. Сервер Denet — персональный control plane, а не единственное место, где «живёт» вся система

Лучшее решение для Denet — **Personal Control Plane with Local-Capable Nodes**:
...
- компьютеры, ноутбуки и телефоны являются не тупыми терминалами, а доверенными или ограниченными **узлами Denet** с локальным кэшем, собственными capabilities, сенсорными источниками, журналом offline-операций и возможностью выполнять работу;
...
Это не классическая cloud-only архитектура и не полностью peer-to-peer multi-master система. Local-first исследования справедливо подчёркивают важность offline-работы, владения данными и отсутствия обязательной зависимости от чужого облака. Но Denet также выполняет внешние действия, управляет разрешениями и запускает долгоживущие процессы — для этого нужен однозначный координационный центр. [[S20]]

Полезной аналогией является разделение control plane и data plane в Tailscale: централизованный координатор распространяет identity и policy, а основной трафик идёт непосредственно между узлами, не превращая сервер в узкое место. Denet не копирует Tailscale, но принимает тот же принцип: **централизовать решения, которым нужна единая власть, и не централизовать тяжёлые данные без необходимости**. [[S19]]

### 0.2. Один активный Head Runtime по умолчанию

Denet не должен начинать с кластера из трёх или пяти согласующихся серверов. Для персональной системы это создаст непропорциональную сложность развертывания, обновлений, диагностики и восстановления.
...
Главная ошибка серверного проектирования Denet состояла бы в попытке синхронизировать всё одинаково.
...
Опыт Figma особенно показателен: даже внутри одного продукта изменения графического документа и изменения comments/users/projects синхронизируются разными системами из-за разных требований к offline availability, security и performance. Denet должен делать то же самое на уровне принципа. [[S21]]
...
В распределённой системе transport может повторить сообщение. Поэтому Denet не обещает магическое `exactly once` для всех каналов.
...
- постоянного существования установки Denet;
...
#### Этап A — восстановить реальные требования Denet
...
**Installation** — одна персональная установка Denet, принадлежащая одному главному пользователю или позднее явно определённой группе principals.
...
### 3.4. Как через сервер проходит обычная работа Denet
...
**Node Runtime** — локальный процесс Denet, работающий на устройстве.  
...
Kubernetes использует leases для heartbeats и отдельно — для leader election; Denet принимает это разделение. [[S25]]
...
Kubernetes leases показывают простой pattern одного активного controller/scheduler среди standby instances. Но полноценная HA всё равно требует инфраструктуры coordination. Для Denet baseline достаточно одного Head, backup и чётко ограниченного failover; correctness-critical multi-node automation можно добавить deployment profile позднее, используя consensus-backed coordination вроде Raft или managed equivalent. [[S25]] [[S28]]
...
Если provider-native session доступна, Denet продолжает её.
...
Denet не обещает бесшовный перенос скрытого внутреннего состояния между vendors.
...
Denet использует небольшой стабильный envelope, близкий по духу к CloudEvents, но не обязан копировать весь стандарт во внутренний hot path. [[S15]]
...
Home Assistant показывает практичную архитектуру event bus + state machine + service registry: occurrence, current state и действие разделены. Denet принимает это разделение, но добавляет trust, durability, semantic triggers и multi-device ordering. [[S18]]
...
Flink разделяет event time и processing time и использует watermarks, чтобы работать с out-of-order/late data. Denet применяет упрощённый принцип: каждый source имеет watermark, а trigger определяет late-event policy. [[S16]]
...
> **Синхронизируется не “вся Denet”, а разные классы данных по разным контрактам.**
...
Denet не пишет собственный универсальный merge engine поверх Git и filesystem. Для Git-проектов authority — Git/worktree. Для обычных папок предпочтителен готовый file-sync backend или explicit copy/versioning.

Syncthing сохраняет conflict copies вместо молчаливого overwriting, потому что sync engine не знает, какая версия лучше пользователю. Denet принимает этот осторожный принцип. [[S24]]
...
Yjs и Automerge демонстрируют mature local-first merge и network-agnostic sync, но добавляют metadata/semantic complexity. Denet подключает подобный backend только там, где real concurrent editing оправдывает его. [[S22]] [[S23]]
...
4. Denet object transfer для artifacts/portable memory;
5. собственный file sync только если существующие решения не дают нужной semantics.

Denet должен знать состояние sync и конфликты, но не обязан реализовывать блоковую репликацию файлов с нуля.
...
- portable exports, если контролируются Denet.
...
AWS рекомендует caller-supplied unique IDs, атомарную фиксацию token вместе с mutation и отклонение повторного ID с другими параметрами. Denet применяет тот же принцип к external effects. [[S12]]
...
GitLab database incident стал известным примером того, как наличие нескольких backup mechanisms не помогло, потому что они не работали как предполагалось и restore не был своевременно проверен. Следствие для Denet: успешный backup job ничего не доказывает без restore test и явного owner. [[S31]]
...
- можно ли запустить safe read-only Denet;
...
Denet не навязывает одинаковые цифры.
...
Provider session может быть непереносима, но Denet-owned state должно позволять:
...
Это позволяет ответить: «почему Denet это сделал и где именно возникла ошибка?» без сохранения скрытого chain-of-thought.
...
- любой project folder должен синхронизироваться Denet;
...
### Внутренние спецификации Denet

**[S01] Denet Functional Concept.** Видение продукта, серверное существование, устройства, backup, интерфейсы и исходные сценарии.  
`00_Denet_Functional_Concept.md`

**[S02] Denet Specification Index and Shared Contracts.** Ownership документов, общие envelopes, sources of truth и границы Server Runtime.  
`01_Denet_Specification_Index_and_Shared_Contracts.md`

**[S03] Denet Memory Fabric 1.2.** Event ledger, evidence, project memory, offline log, consistency, deletion и portable memory.  
`10_Denet_Memory_Fabric.md`

**[S04] Denet Pragmatic Agentic Control Fabric 1.1.** Strong-agent-first модель, Managed Run, proportional durability, checkpoints и external effects.  
`20_Denet_Agentic_Control_Fabric.md`

**[S05] Denet Trust, Identity, Autonomy and Permissions.** Device trust, Reference Monitor, grants, Secret Broker, computer-use boundaries и effect authorization.  
`30_Denet_Trust_Identity_Autonomy_and_Permissions.md`

**[S06] Denet Capabilities, Providers and Integrations 1.1.** Provider/runtime lifecycle, connectors, device capabilities, health, quota и fallback.  
...
> **Server Runtime Denet должен быть единым логическим персональным control plane, но не единственной вычислительной и файловой машиной. Он сохраняет authoritative coordination state, долговечно поддерживает только ту работу, которой это нужно, принимает и фильтрует события, синхронизирует разные данные по различным контрактам, предотвращает повтор внешних эффектов, честно ограничивает offline и split-brain режимы, проверяет backups восстановлением и позволяет перенести систему на другое доверенное устройство без потери пользовательской памяти и проектов.**
