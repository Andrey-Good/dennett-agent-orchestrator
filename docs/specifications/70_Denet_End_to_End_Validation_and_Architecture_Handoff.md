# Denet End-to-End Validation and Architecture Handoff
...
> Related: [60_Denet_Desktop_Application_Business_Logic.md](./60_Denet_Desktop_Application_Business_Logic.md) · [61_Denet_Mobile_Application_Business_Logic.md](./61_Denet_Mobile_Application_Business_Logic.md).
...
**Каноническое имя:** `70_Denet_End_to_End_Validation_and_Architecture_Handoff.md`.
...
Принятые документы уже определяют ядро Denet достаточно глубоко:
...
- **OUT OF SCOPE** — функция не входит в текущую личную установку Denet;
...
Архитектурный риск: provider-native sessions и Denet canonical state не должны расходиться.
...
- change remote;
- archive;
- remove from Denet without deleting files;
...
Любой значимый путь Denet должен раскладываться так:
...
### INST-05. Обновление Denet при разных версиях clients
...
Denet обнаруживает Git, инструкции, project memory, capabilities и workspace trust. До доверия код не исполняется. После Trusted-Bounded обычная работа идёт без постоянных prompts.

### PRJ-03. Продолжение проекта голосом

Пользователь говорит: «Продолжи Denet, разберись с последней ошибкой». Voice создаёт Intent Proposal; Orchestrator находит project/session; project agent получает context и работает в своей директории.
...
UI показывает Missing. User может Locate, Rebind, Clone Again, Keep Metadata или Remove from Denet. Никакой memory/project history не удаляется автоматически.
...
Denet видит diff, пересобирает Effective Instruction Set, отмечает stale projections и не перезаписывает human content молча.
...
Task не теряется. Denet создаёт новую provider session из checkpoint/Context Manifest и маркирует возможный semantic drift.
...
Meeting capture требует соответствующей policy/consent. Denet должен поддержать visual indicator, participant notice configuration и source exclusion. Конкретные правовые правила зависят от юрисдикции и требуют отдельной проверки перед эксплуатацией.
...
### COM-07. User edits Denet’s draft
...
Task remains in Denet state; wait/retry/fallback/local/partial/cancel are explicit.
...
### 26.1. `80_Denet_System_Architecture_and_Runtime_Topology.md`

**Вопрос:** из каких исполняемых частей состоит Denet и как они живут вместе?
...
### 26.2. `81_Denet_Data_Memory_Storage_Sync_and_Protocol_Architecture.md`
...
### 26.3. `82_Denet_Agent_Voice_Capability_and_Integration_Architecture.md`
...
### 26.4. `83_Denet_Client_Operations_Testing_and_Implementation_Blueprint.md`
