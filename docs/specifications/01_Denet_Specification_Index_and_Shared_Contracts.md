# Denet Specification Index and Shared Contracts
...
> Related: [00_Denet_Functional_Concept.md](./00_Denet_Functional_Concept.md).
...
По мере развития Denet одни и те же слова начали встречаться в нескольких местах: «агент», «задача», «событие», «память проекта», «разрешение», «сессия», «результат», «состояние». Без единой карты поздние документы неизбежно начнут:
...
Этот файл предотвращает расхождение, но сам не должен стать бюрократическим центром. Его задача — закрепить **несколько общих инвариантов и лёгкие границы**, а не заставить каждое действие Denet проходить через один гигантский универсальный протокол.
...
**Текущий исходный файл:** `00_Denet_Functional_Concept.md`  
**Владеет:** видением продукта, ценностями, пользовательскими возможностями, общей моделью Denet и намерениями владельца проекта.  
...
**Текущий исходный файл:** `10_Denet_Memory_Fabric.md`  
...
**Текущий исходный файл:** `20_Denet_Agentic_Control_Fabric.md`  
...
- `Denet_memory_logic_v0.2.md`;
- `Denet_memory_logic_v1.0_memory_fabric_2026.md`;
- `Denet_memory_logic_v1.1_adaptive_federated_2026.md`;
- `Denet_agent_orchestration_logic_v1.0_agentic_control_fabric_2026.md`;
- промежуточные версии функциональной концепции;
- `Denet_функциональная_концепция_v0.1.2_terms_and_codex_base.md`, если она расходится с пользовательской редакцией `project_workflow(2)`;
...
Версия файла не обязана совпадать с версией продукта Denet.
...
Единственный логический управляющий центр Denet. Он принимает намерения и события, выбирает форму работы, управляет глобальными и межпроектными решениями, но не является одним бесконечным prompt-контекстом и не исполняет каждое локальное действие.
...
Главный человеческий principal личной установки Denet. Пользователь является владельцем целей и данных, но конкретное действие всё равно требует подтверждённой identity/session и соблюдения safety floor.
...
Ограниченный во времени и контексте канал взаимодействия principal с Denet. Типы session уточняются доменными документами: user session, voice session, project session, provider session.
...
То, чем Denet может воспользоваться: модель, provider, tool, MCP, plugin, connector, computer-use, speech, vision, web search, приложение или device function.
...
Пересобираемое представление того, что Denet должен считать актуальным для конкретного key/scope сейчас.
...
# 6. Общие инварианты Denet

## 6.1. Пользовательское намерение и no-op

1. Denet стремится понять реальную цель пользователя, а не только буквальную форму команды.
...
- Denet-native instruction records;
...
Denet нормализует только необходимую общую границу.
...
- окончательное имя скрытой папки Portable Project Memory Pack (`.denet/memory/` пока рабочее);
- окончательные canonical filenames в репозитории;
- naming policy для русских и английских терминов;
- официальное написание Denet во всех интерфейсах.
...
7. В Denet одна логическая Memory Fabric. Client, local-only и server deployment используют разные физические adapters, а не разные значения понятия памяти.
