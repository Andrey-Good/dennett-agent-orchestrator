...
`just check`; branch protection требует этот check на актуальном `main`. Владелец репозитория
сохраняет явный emergency bypass, поэтому обычное прохождение gate не зависит от скрытого
provider token или отдельного CI-сценария.
